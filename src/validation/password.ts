...
export const PASSWORD_RULES_TEXT =
  'A senha deve ter no mínimo 8 caracteres e incluir ao menos uma letra maiúscula, ' +
  'uma letra minúscula, um número e um caractere não alfanumérico (ex: !, @, #).'
...
export function validatePassword(password: string): string[] {
  ...
  if (password.length < PASSWORD_MIN_LENGTH)
    problems.push(`A senha deve ter no mínimo ${PASSWORD_MIN_LENGTH} caracteres.`)
  if (!/[A-Z]/.test(password)) problems.push('A senha deve incluir uma letra maiúscula.')
  if (!/[a-z]/.test(password)) problems.push('A senha deve incluir uma letra minúscula.')
  if (!/[0-9]/.test(password)) problems.push('A senha deve incluir um número.')
  if (!/[^a-zA-Z0-9]/.test(password))
    problems.push('A senha deve incluir um caractere não alfanumérico.')
